...
# authn creds from env variables or /etc/conjur* files
...
get_authn_info() {
	...
	if [[ "$CONJUR_APPLIANCE_URL" == "" ]]; then
		if [[ ! -e /etc/conjur.conf ]]; then
			echo "CONJUR_APPLIANCE_URL not set and /etc/conjur.conf not found."
			exit
		fi
		CONJUR_APPLIANCE_URL=$(cat /etc/conjur.conf | awk '/appliance_url/ {print $2}')
	fi
	if [[ "$CONJUR_CERT_FILE" == "" ]]; then
		if [[ ! -e /etc/conjur.conf ]]; then
			echo "CONJUR_CERT_FILE not set and /etc/conjur.conf not found."
			exit
		fi
		CONJUR_CERT_FILE=$(cat /etc/conjur.conf | awk '/cert_file/ {print $2}')
	fi
	CONJUR_CERT_FILE=$(sed -e 's/^"//' -e 's/"$//' <<<$CONJUR_CERT_FILE)		# remove quotes, if any
	if [[ "$CONJUR_AUTHN_LOGIN" == "" ]]; then
		if [[ ! -e /etc/conjur.identity ]]; then
			echo "CONJUR_AUTHN_LOGIN not set and /etc/conjur.identity not found."
			exit
		fi
	...
	if [[ "$CONJUR_AUTHN_API_KEY" == "" ]]; then
		if [[ ! -e /etc/conjur.identity ]]; then
			echo "CONJUR_AUTHN_API_KEY not set and /etc/conjur.identity not found."
			exit
		fi
...
host_authn() {
	...
	# Authenticate host w/ its name & API key to get session token
	 ...
	 $CONJUR_APPLIANCE_URL/authn/dev/$host_name/authenticate)
...
fetch_secret() {
	...
	SECRET_VALUE=$(curl -s \
	 --cacert $CONJUR_CERT_FILE \
         ...
         $CONJUR_APPLIANCE_URL/secrets/dev/variable/$var_id_urlfmt)
}
